...
<link rel="canonical" href="http://localhost:4000/node_modules/node-sass/src/libsass/docs/unicode.html" />
<meta property="og:url" content="http://localhost:4000/node_modules/node-sass/src/libsass/docs/unicode.html" />
<meta property="og:site_name" content="estebang23.github.io" />
<script type="application/ld+json">
{"@type":"WebPage","url":"http://localhost:4000/node_modules/node-sass/src/libsass/docs/unicode.html","headline":"estebang23.github.io","@context":"http://schema.org"}</script>
<!-- End Jekyll SEO tag -->
  ...
  <body>
    <div class="container-lg px-3 my-5 markdown-body">
      ...
      <p>LibSass currently expects all input to be utf8 encoded (and outputs only utf8), if you actually have any unicode characters at all. We do not support conversion between encodings, even if you declare it with a <code class="highlighter-rouge">@charset</code> rule. The text below was originally posted as an <a href="https://github.com/sass/libsass/issues/381">issue</a> on the LibSass tracker. Since then the status is outdated as LibSass now expects your
...
behavior, which can in the worst case lead to buffer overruns/segfaults. Therefore LibSass now checks your input to be valid utf8 encoded!</p>
...
<p>LibSass currently reads all kind of BOMs and will error out if it finds something it doesn’t know how to handle! It seems that it throws away the optional UTF-8 BOM (if any is found). IMO it would be nice if users could configure that (also if a charset rule should be added to the output). But it does not really take any <code class="highlighter-rouge">@charset</code> into account, it always assumes your input is utf8 and ignores any given <code class="highlighter-rouge">@charset</code>!</p>
...
<p>I guess the current implementation should handle more than 99% of all real world use cases.
