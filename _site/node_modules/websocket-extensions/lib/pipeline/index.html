...
<title>Extension pipelining | estebang23.github.io</title>
<meta name="generator" content="Jekyll v3.7.3" />
<meta property="og:title" content="Extension pipelining" />
<meta property="og:locale" content="en_US" />
<link rel="canonical" href="http://localhost:4000/node_modules/websocket-extensions/lib/pipeline/" />
<meta property="og:url" content="http://localhost:4000/node_modules/websocket-extensions/lib/pipeline/" />
<meta property="og:site_name" content="estebang23.github.io" />
<script type="application/ld+json">
{"@type":"WebPage","url":"http://localhost:4000/node_modules/websocket-extensions/lib/pipeline/","headline":"Extension pipelining","@context":"http://schema.org"}</script>
<!-- End Jekyll SEO tag -->
...
<p><code class="highlighter-rouge">websocket-extensions</code> models the extension negotiation and processing pipeline
...
outgoing from the application, for example the <code class="highlighter-rouge">permessage-deflate</code> extension
...
<div class="highlighter-rouge"><div class="highlight"><pre class="highlight"><code>+-------------+  out  +---+     +---+     +---+       +--------+
|             |------&gt;|   |----&gt;|   |----&gt;|   |------&gt;|        |
...
<div class="highlighter-rouge"><div class="highlight"><pre class="highlight"><code>    C.outgoing(B.outgoing(A.outgoing(message)))
...
<div class="highlighter-rouge"><div class="highlight"><pre class="highlight"><code>    A.incoming(B.incoming(C.incoming(message)))
...
<div class="highlighter-rouge"><div class="highlight"><pre class="highlight"><code>type Session = {
...
<p>(That <code class="highlighter-rouge">() -&gt; ()</code> syntax is intended to mean that <code class="highlighter-rouge">close()</code> is a nullary void
method; I apologise to any Haskell readers for not using the right monad.)</p>

<p>The <code class="highlighter-rouge">incoming()</code> and <code class="highlighter-rouge">outgoing()</code> methods perform message transformation in the
respective directions; <code class="highlighter-rouge">close()</code> is called when a socket closes so the session
can release any resources it’s holding, for example a DEFLATE de/compression
context.</p>

<p>However because this is JavaScript, the <code class="highlighter-rouge">incoming()</code> and <code class="highlighter-rouge">outgoing()</code> methods
may be asynchronous (indeed, <code class="highlighter-rouge">permessage-deflate</code> is based on <code class="highlighter-rouge">zlib</code>, whose API
...
wire. <code class="highlighter-rouge">message</code> is a value conforming the type:</p>

<div class="highlighter-rouge"><div class="highlight"><pre class="highlight"><code>type Message = {
  rsv1   :: Boolean
  rsv2   :: Boolean
  rsv3   :: Boolean
  opcode :: Number
...
<div class="language-js highlighter-rouge"><div class="highlight"><pre class="highlight"><code><span class="kd">var</span> <span class="nx">zlib</span> <span class="o">=</span> <span class="nx">require</span><span class="p">(</span><span class="s1">'zlib'</span><span class="p">);</span>
...
<div class="language-js highlighter-rouge"><div class="highlight"><pre class="highlight"><code><span class="kd">var</span> <span class="nx">crypto</span> <span class="o">=</span> <span class="nx">require</span><span class="p">(</span><span class="s1">'crypto'</span><span class="p">),</span>
    ...
    <span class="nx">small</span>  <span class="o">=</span> <span class="k">new</span> <span class="nx">Buffer</span><span class="p">(</span><span class="s1">'hi'</span><span class="p">);</span>
...
<span class="nx">deflate</span><span class="p">.</span><span class="nx">outgoing</span><span class="p">({</span><span class="na">data</span><span class="p">:</span> <span class="nx">large</span><span class="p">},</span> <span class="kd">function</span><span class="p">()</span> <span class="p">{</span>
  <span class="nx">console</span><span class="p">.</span><span class="nx">log</span><span class="p">(</span><span class="mi">1</span><span class="p">,</span> <span class="s1">'large'</span><span class="p">);</span>
<span class="p">});</span>
...
<span class="nx">deflate</span><span class="p">.</span><span class="nx">outgoing</span><span class="p">({</span><span class="na">data</span><span class="p">:</span> <span class="nx">small</span><span class="p">},</span> <span class="kd">function</span><span class="p">()</span> <span class="p">{</span>
  <span class="nx">console</span><span class="p">.</span><span class="nx">log</span><span class="p">(</span><span class="mi">2</span><span class="p">,</span> <span class="s1">'small'</span><span class="p">);</span>
<span class="p">});</span>
...
<p>For example, when using <code class="highlighter-rouge">permessage-deflate</code> without <code class="highlighter-rouge">no_context_takeover</code> set,
...
<p>For example, it is entirely possible to take the simple <code class="highlighter-rouge">deflate</code> extension
above and wrap its <code class="highlighter-rouge">incoming()</code> and <code class="highlighter-rouge">outgoing()</code> methods in two <code class="highlighter-rouge">Transform</code>
...
<p>The <code class="highlighter-rouge">Transform</code> class makes it easy to wrap an async function such that message
order is preserved:</p>

<div class="language-js highlighter-rouge"><div class="highlight"><pre class="highlight"><code><span class="kd">var</span> <span class="nx">stream</span>  <span class="o">=</span> <span class="nx">require</span><span class="p">(</span><span class="s1">'stream'</span><span class="p">),</span>
...
<code class="highlighter-rouge">callback()</code> until the async function has ‘returned’, which blocks <code class="highlighter-rouge">Transform</code>
from passing further input into the <code class="highlighter-rouge">_transform()</code> method until the current
...
<p>A driver using this framework holds an instance of the <code class="highlighter-rouge">Extensions</code> class, which
it uses to register extension plugins, negotiate headers and transform messages.
The <code class="highlighter-rouge">Extensions</code> instance itself holds a <code class="highlighter-rouge">Pipeline</code>, which contains an array of
<code class="highlighter-rouge">Cell</code> objects, each of which wraps one of the sessions.</p>

<h3 id="message-processing">Message processing</h3>

<p>Both the <code class="highlighter-rouge">Pipeline</code> and <code class="highlighter-rouge">Cell</code> classes have <code class="highlighter-rouge">incoming()</code> and <code class="highlighter-rouge">outgoing()</code>
methods; the <code class="highlighter-rouge">Pipeline</code> interface pushes messages into the pipe, delegates the
message to each <code class="highlighter-rouge">Cell</code> in turn, then returns it back to the driver. Outgoing
messages pass through <code class="highlighter-rouge">A</code> then <code class="highlighter-rouge">B</code> then <code class="highlighter-rouge">C</code>, and incoming messages in the
reverse order.</p>

<p>Internally, a <code class="highlighter-rouge">Cell</code> contains two <code class="highlighter-rouge">Functor</code> objects. A <code class="highlighter-rouge">Functor</code> wraps an async
...
between input and output messages. To use our simple <code class="highlighter-rouge">deflate</code> extension from
above:</p>

<div class="language-js highlighter-rouge"><div class="highlight"><pre class="highlight"><code><span class="kd">var</span> <span class="nx">functor</span> <span class="o">=</span> <span class="k">new</span> <span class="nx">Functor</span><span class="p">(</span><span class="nx">deflate</span><span class="p">,</span> <span class="s1">'outgoing'</span><span class="p">);</span>
...
<p>A <code class="highlighter-rouge">Cell</code> contains two of these, one for each direction:</p>
...
<p>This satisfies the message transformation requirements: the <code class="highlighter-rouge">Pipeline</code> simply
loops over the cells in the appropriate direction to transform each message.
Because each <code class="highlighter-rouge">Cell</code> will preserve message order, we can pass a message to the
next <code class="highlighter-rouge">Cell</code> in line as soon as the current <code class="highlighter-rouge">Cell</code> returns it. This gives each
<code class="highlighter-rouge">Cell</code> all the messages in order while maximising throughput.</p>

<h3 id="session-closing">Session closing</h3>

<p>We want to close each session as soon as possible, after all existing messages
have drained. To do this, each <code class="highlighter-rouge">Cell</code> begins with a pending message counter in
each direction, labelled <code class="highlighter-rouge">in</code> and <code class="highlighter-rouge">out</code> below.</p>
...
<p>When a message <em>m1</em> enters the pipeline, say in the <code class="highlighter-rouge">outgoing</code> direction, we
increment the <code class="highlighter-rouge">pending.out</code> counter on all cells immediately.</p>
...
<p><em>m1</em> is handed off to <code class="highlighter-rouge">A</code>, meanwhile a second message <code class="highlighter-rouge">m2</code> arrives in the same
direction. All <code class="highlighter-rouge">pending.out</code> counters are again incremented.</p>
...
<p>When the first cell’s <code class="highlighter-rouge">A.outgoing</code> functor finishes processing <em>m1</em>, the first
<code class="highlighter-rouge">pending.out</code> counter is decremented and <em>m1</em> is handed off to cell <code class="highlighter-rouge">B</code>.</p>
...
<p>As <code class="highlighter-rouge">B</code> finishes with <em>m1</em>, and as <code class="highlighter-rouge">A</code> finishes with <em>m2</em>, the <code class="highlighter-rouge">pending.out</code>
...
<p>Say <code class="highlighter-rouge">C</code> is a little slow, and begins processing <em>m2</em> while still processing
<em>m1</em>. That’s fine, the <code class="highlighter-rouge">Functor</code> mechanism will keep <em>m1</em> ahead of <em>m2</em> in the
output.</p>
...
<p>Once all messages are dealt with, the counters return to <code class="highlighter-rouge">0</code>.</p>
...
<p>The same process applies in the <code class="highlighter-rouge">incoming</code> direction, the only difference being
that messages are passed to <code class="highlighter-rouge">C</code> first.</p>

<p>This makes closing the sessions quite simple. When the driver wants to close the
socket, it calls <code class="highlighter-rouge">Pipeline.close()</code>. This <em>immediately</em> calls <code class="highlighter-rouge">close()</code> on all
the cells. If a cell has <code class="highlighter-rouge">in == out == 0</code>, then it immediately calls
<code class="highlighter-rouge">session.close()</code>. Otherwise, it stores the closing call and defers it until
<code class="highlighter-rouge">in</code> and <code class="highlighter-rouge">out</code> have both ticked down to zero. The pipeline will not accept new
messages after <code class="highlighter-rouge">close()</code> has been called, so we know the pending counts will not
increase after this point.</p>

<p>This means each session is closed as soon as possible: <code class="highlighter-rouge">A</code> can close while the
slow <code class="highlighter-rouge">C</code> session is still working, because it knows there are no more messages
on the way. Similarly, <code class="highlighter-rouge">C</code> will defer closing if <code class="highlighter-rouge">close()</code> is called while <em>m1</em>
is still in <code class="highlighter-rouge">B</code>, and <em>m2</em> in <code class="highlighter-rouge">A</code>, because its pending count means it knows it
...
<code class="highlighter-rouge">Pipeline</code> holds a list of <code class="highlighter-rouge">Cell</code> objects, each wrapping a <code class="highlighter-rouge">Session</code>. The driver
talks (via the <code class="highlighter-rouge">Extensions</code> API) to the <code class="highlighter-rouge">Pipeline</code> interface, and it wants
<code class="highlighter-rouge">Pipeline.close()</code> to do two things: close all the sessions, and tell me when
...
<p>At the other end of the stack, <code class="highlighter-rouge">Session.close()</code> is a nullary void method with
...
<p>In the middle, <code class="highlighter-rouge">Cell.close()</code> returns a promise rather than using a callback.
This is for two reasons. First, <code class="highlighter-rouge">Cell.close()</code> might not do anything
...
purposes of <code class="highlighter-rouge">Pipeline.close()</code>: it wants to call <code class="highlighter-rouge">close()</code> on each of a list of
...
<p>At this point, session <code class="highlighter-rouge">B</code> produces an error while processing <em>m2</em>, that is <em>m2</em>
...
messages. Session <code class="highlighter-rouge">B</code> might have begun processing <em>m3</em> by the time it produces
the error <em>e2</em>, but <code class="highlighter-rouge">C</code> will have been given <em>e2</em> before it receives <em>m3</em>, and
can simply drop <em>m3</em>.</p>

<p>Now, say <em>e2</em> reaches the slow session <code class="highlighter-rouge">C</code> while <em>m1</em> is still present,
meanwhile <em>m3</em> has been dropped. <code class="highlighter-rouge">C</code> will never receive <em>m3</em> since it will have
been dropped upstream. Under the present model, its <code class="highlighter-rouge">out</code> counter will be <code class="highlighter-rouge">3</code>
but it is only going to emit two more values: <em>m1</em> and <em>e2</em>. In order for
closing to work, we need to decrement <code class="highlighter-rouge">out</code> to reflect this. The situation
...
<p>Note that while <em>e2</em> is in the pipeline, <code class="highlighter-rouge">Pipeline</code> should drop any further
...
pipeline following <em>m2</em> will be processed by <code class="highlighter-rouge">A</code>, since it’s upstream of the
error. Those messages will be dropped by <code class="highlighter-rouge">B</code>.</p>

<h2 id="alternative-ideas">Alternative ideas</h2>

<p>I am considering implementing <code class="highlighter-rouge">Functor</code> as an object-mode transform stream
